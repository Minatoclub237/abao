...
export async function generateMetadata({ params }: { params: Promise<{ lieu: string }> }): Promise<Metadata> {
  ...
  if (!l) return {};
  return {
    title: `Serrurier ${l.nom} (${l.cp}) — intervention en ${l.delai}, 24h/24`,
    description: `Serrurier à ${l.nom} : intervention en ${l.delai} environ, 24h/24 et 7j/7. Devis gratuit, agréé assurances. Quartiers ${l.quartiers.slice(0, 4).join(', ')}. Un seul numéro : 06 60 09 49 76.`,
  };
}
...
export default async function Page({ params }: { params: Promise<{ lieu: string }> }) {
  const l = lieuBySlug((await params).lieu);
  if (!l) notFound();
  return <LocalPage service="serrurerie" lieu={l} />;
}
